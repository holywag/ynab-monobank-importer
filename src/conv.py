...
def request_ex_rates(date_start: dt.date, date_stop: dt.date, currency: str) -> dict[dt.date, float]:
...
def init_rates_cache(date_start: dt.date, date_stop: dt.date):
    cache = {}
    try:
...
rates = init_rates_cache(inflow_dates[0], inflow_dates[-1])
